...
export function initPostGreSQLDB(pool: Pool): void {
  ...
  pool.query(
    "CREATE TABLE IF NOT EXISTS balance (id SERIAL PRIMARY KEY, userId INT NOT NULL, balance INT NOT NULL,currency VARCHAR(3) NOT NULL,  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
  );
}
